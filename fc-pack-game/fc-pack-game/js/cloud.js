...
const Cloud = {
  ...
  async loadContent() {
    const r = await fetch(this.base + '/api/content', { cache: 'no-store' });
    if (!r.ok) throw new Error('content');
    return await r.json();
  },

  async saveContent(username, password, content) {
    const r = await fetch(this.base + '/api/content', {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username, password, content })
    });
    const j = await r.json().catch(() => ({}));
    if (!r.ok) throw new Error(j.error || 'Save content failed');
    return true;
  },
